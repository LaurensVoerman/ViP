@cd /D "%~dp0"
@SET "UE_4=C:\Program Files\Epic Games\UE_4.27\Engine\Binaries\Win64\UE4Editor.exe"
@FOR /F "usebackq tokens=1,2* " %%i in (`reg QUERY "HKLM\SOFTWARE\EpicGames\Unreal Engine\4.27" /v "InstalledDirectory" 2^> NUL`) do @(
@if %%i==InstalledDirectory @if %%j==REG_SZ set "UE_4=%%k\Engine\Binaries\Win64\UE4Editor.exe"
)
@if not exist "%UE_4%" @(
@echo UE4Editor not found; "%UE_4%"
@GOTO :EOF
)
@echo UE4Editor found:  "%UE_4%"
rem fake builddata
...
start "UE4" /b "%UE_4%" "%~dp0%%i" -ExecCmds="t.MaxFPS 30"
